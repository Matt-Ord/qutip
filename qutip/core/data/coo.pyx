...
cpdef inline size_t nnz(COO matrix) noexcept nogil:
    """Get the number of non-zero elements of a COO matrix."""
    return matrix._nnz


cpdef COO empty(base.idxint rows, base.idxint cols, size_t size):
    """
...
cpdef COO expand(COO matrix, size_t size):
...
cpdef COO identity(base.idxint dimension, double complex scale=1):
    ...
    """
    cdef COO out = empty(dimension, dimension, <size_t > dimension)
    cdef base.idxint i
...
cpdef COO from_dense(Dense matrix):
    ...
    cdef base.idxint row, col, row_stride, col_stride
    cdef size_t ptr_in, ptr_out=0
